...
-- Standalone, append-only record of every wallet address the bot has ever generated or
-- used, independent of the trades table. Even if a trade record is ever lost/corrupted,
-- this keeps a durable log of exactly which (coin, derivation index) produced which
-- address, and what it was for — so nothing about where funds are is ever solely reliant
-- on the trades table staying intact.
CREATE TABLE IF NOT EXISTS wallet_log (
  id                SERIAL PRIMARY KEY,
  coin              TEXT NOT NULL,          -- BTC | LTC | ETH | SOL | USDT_BEP20
  derivation_index  INTEGER NOT NULL,
  address           TEXT NOT NULL,
  purpose           TEXT NOT NULL,          -- 'deposit' (per-trade) | 'hot_wallet'
  trade_id          INTEGER,                -- nullable, set for purpose='deposit'
  created_at        TIMESTAMPTZ NOT NULL DEFAULT now(),
